...
# monorepo and the server imports `@arena/shared` as a workspace dependency;
...
#   docker run -e PORT=2601 -p 2601:2601 arena-server
...
# imports require: `@arena/shared` resolves to `shared/dist/index.js`.
RUN npm run build:server

# Drop to production dependencies in place. This keeps the workspace symlinks
# that `@arena/shared` resolves through - deleting node_modules and
...
# hosts, injects one - and `serverConfig` falls back to 2601.
ENV HOST=0.0.0.0
...
HEALTHCHECK --interval=30s --timeout=5s --start-period=15s --retries=3 \
  CMD node -e "fetch('http://127.0.0.1:'+(process.env.PORT||2601)+'/health').then(r=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))"
