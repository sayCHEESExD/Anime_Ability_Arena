...
import { COMBAT, visibleName } from '@arena/shared';
...
const FACE_SHARE = 0.46;
/** Share of the plate the health bar takes, under the name. */
const HEALTH_SHARE = 0.2;
...
export class NamePlate {
  ...
  private name = '';
  private face = '';
  /** Health as drawn (-1 = no bar: on the spawn island). */
  private health = -1;
   ...
   */
  /** The health bar under the name; pass -1 to hide it. */
  setHealth(hp: number): void {
    const value = hp < 0 ? -1 : Math.round(Math.max(0, Math.min(COMBAT.maxHp, hp)));
  ...
  private repaint(): void {
    ...
    const signature = `${this.name}\u0000${image ? this.face : ''}\u0000${this.health}`;
  ...
  private paint(image: HTMLImageElement | null): void {
     ...
     */
    const strengthBand = this.health >= 0 ? height * HEALTH_SHARE : 0;
    const faceSize = image ? height * FACE_SHARE : 0;
    const textBand = height - faceSize - strengthBand;
    ...
    if (this.health >= 0) {
      // The health bar: a dark trough, filled green to red.
      const barW = width * 0.62;
      const barH = strengthBand * 0.62;
      const x = (width - barW) / 2;
      const y = faceSize + textBand + (strengthBand - barH) / 2;
      const f = this.health / COMBAT.maxHp;
      ctx.fillStyle = 'rgba(8, 10, 18, 0.85)';
      ctx.fillRect(x - 3, y - 3, barW + 6, barH + 6);
      ctx.fillStyle = f > 0.5 ? '#46e05a' : f > 0.25 ? '#ffc93d' : '#ff4040';
      ctx.fillRect(x, y, barW * f, barH);
      ctx.fillStyle = 'rgba(255,255,255,0.25)';
      ctx.fillRect(x, y, barW * f, barH * 0.35);
    }
