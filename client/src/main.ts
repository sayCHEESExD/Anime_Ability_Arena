...
const main = async (): Promise<void> => {
   ...
   */
  setBootStatus('Entering the arena…');
  await paint();
  ...
  setBootStatus('Building the islands…');
  game.loadingStep('Building the islands…');
  await paint();
  ...
  if (clientConfig.debug) {
    ...
    (window as Window & { __arena?: DebugHandle }).__arena = { game, loop };
    void import('./debug/harness.js').then(({ installHarness }) => installHarness(game, loop));
  }
  ...
  if (bootStatus) {
    ...
    bootStatus.textContent = clientConfig.serverUrl
      ? `Not connected to the game server (${clientConfig.serverUrl}).\n` +
        'Playing offline: Yen, kills and kits are server-owned and ' +
...
/** Shape of the dev-only `window.__arena` handle. */
