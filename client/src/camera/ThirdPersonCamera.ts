import { CAMERA, type WorldCollision } from '@arena/shared';
...
export class ThirdPersonCamera {
   ...
   */
  /** The world the camera must stay out of. */
  private collision: WorldCollision | null = null;
  /** 0..1 of the orbit distance the camera may use, eased back out after a wall. */
  private clearance = 1;

  /** Keep the camera out of this world's solids (a pedestal, a shop, a wall). */
  setCollision(collision: WorldCollision): void {
  ...
  update(delta: number, speed: number): void {
    ...
    // Never inside a solid: pull in to the first one between the player and the
    // camera at once, and ease back out once it is clear.
    if (this.collision) {
      const p = this.camera.position;
      const hit = this.collision.segmentClear(LOOK_TARGET.x, LOOK_TARGET.y, LOOK_TARGET.z, p.x, p.y, p.z);
      const want = hit < 1 ? Math.max(0.08, hit - 0.06) : 1;
      this.clearance = want < this.clearance ? want : this.clearance + (want - this.clearance) * (1 - Math.exp(-4 * delta));
      if (this.clearance < 0.999) p.lerpVectors(LOOK_TARGET, p, this.clearance);
    }
